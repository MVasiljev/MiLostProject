 ...
 */
export function createTuple(req, res) {
    try {
 ...
 */
export function getTupleItem(req, res) {
    try {
        const { values, index } = req.body;
        if (!values || !Array.isArray(values)) {
            return res.status(400).json({
                error: "Tuple values array is required",
            });
 ...
 */
export function getFirstItem(req, res) {
    try {
        const { values } = req.body;
 ...
 */
export function getSecondItem(req, res) {
    try {
        const { values } = req.body;
 ...
 */
export function replaceTupleItem(req, res) {
    try {
        const { values, index, value } = req.body;
        if (!values || !Array.isArray(values)) {
            return res.status(400).json({
                error: "Tuple values array is required",
            });
 ...
 * Map a tuple
 */
export function mapTuple(req, res) {
    try {
 ...
 * Get tuple length
 */
export function getTupleLength(req, res) {
    try {
        const { values } = req.body;
        if (!values || !Array.isArray(values)) {
            return res.status(400).json({
 ...
 */
export function tupleOperations(req, res) {
    try {
        const { values, operation, index, value, mapOperation } = req.body;
        ...
        switch (operation) {
            case "get":
                ...
                return getTupleItem({ ...req, body: { values, index } }, res);
            case "first":
                return getFirstItem({ ...req, body: { values } }, res);
            case "second":
                return getSecondItem({ ...req, body: { values } }, res);
            case "replace":
                ...
                return replaceTupleItem({ ...req, body: { values, index, value } }, res);
            case "map":
                if (!mapOperation) {
                    return res.status(400).json({
                ...
                return mapTuple({ ...req, body: { values, operation: mapOperation } }, res);
            case "length":
                return getTupleLength({ ...req, body: { values } }, res);
            default:
                return res.status(400).json({
 ...
 */
export function analyzeTuple(req, res) {
    try {
