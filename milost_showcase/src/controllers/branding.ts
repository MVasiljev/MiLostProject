...
export function createBranded(req: Request, res: Response): Response {
  try {
    ...
    try {
      ...
      return res.status(400).json({
        error: `Failed to create validator function: ${error instanceof Error ? error.message : String(error)}`,
      });
    }
    ...
    const response: CreateBrandedResponse = {
      data: {
        ...
        error: result.isErr() ? result.unwrapErr().message : null,
      },
    };
...
export function unwrapBranded(req: Request, res: Response): Response {
  try {
    ...
    if (brandedResult.isErr()) {
      return res.status(400).json({
        error: `Failed to create branded value: ${brandedResult.unwrapErr().message}`,
      });
    }
...
export function convertBranded(req: Request, res: Response): Response {
  try {
    ...
    if (sourceResult.isErr()) {
      return res.status(400).json({
        error: `Failed to create source branded value: ${sourceResult.unwrapErr().message}`,
      });
    }
    ...
    const response: ConvertBrandedResponse = {
      data: {
        ...
        error: targetResult.isErr() ? targetResult.unwrapErr().message : null,
      },
    };
